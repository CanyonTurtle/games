...
- **`kernel.js` and `fixtures.md`** (`DESIGN.md` §26), from direct
  feedback relayed by another agent that tried to author a cart here
  with no prior context and couldn't: the real format only existed as
  running code inside this file, it had no browser or network access
  to fetch or execute that code, and no way to validate a hand-encoded
  cart short of pasting it back into the live page. `kernel.js` is
  `encodeCart`/`decodeCart`/the opcode table/`assemble()`/`runHook()`/
  the compression transport, copied byte-for-byte out of this file into
  one dependency-free module — no DOM, runs under plain Node (or any JS
  host) with zero network access, verified byte-identical to this
  file's own functions across every shipped cart plus cross-runtime
  compression interop (`test/check-kernel-sync.js`). `fixtures.md` is a
  handful of known-good (cart → exact bytes → exact fragment) examples
  generated by it, including one that runs a real hook through the VM
  and shows the resulting globals — a way to check an implementation
  against ground truth without running anything at all. Kept in sync
  by discipline, not tooling: it's a copy, not an import, since this
