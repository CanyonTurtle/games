...
## 103. Open Relay Project's static TURN credentials went dark — real two-device connections started failing again, with the exact same "after exchanging SDP" signature §95 had just fixed

A real two-device party hit repeated `could not connect to peer X after exchanging SDP` failures over two-plus minutes of live use — signaling (tracker relays reaching OPEN, the whole §85-92 arc) working fine, only the actual WebRTC/ICE connection failing, the identical signature §95 diagnosed and fixed with a TURN relay. Confirmed directly by reading `vendor/trystero/peer.mjs`/`signal-handler.mjs` that the wiring itself was still correct (`turnConfig` still reaching `RTCPeerConnection`, the error message only fires when Trystero detects a `turnConfig` *is* present) — so the regression wasn't in this project's code. A web search pinned the actual cause: metered.ca changed the Open Relay Project's deployment model since §95 shipped. The static, publicly-embeddable `openrelayproject`/`openrelayproject` credentials no longer authenticate at all — an account and their TURN REST API (short-lived, dynamically-minted credentials) is now required, even for the free tier.

This is a real product tradeoff, not just a bug fix — §95's whole rationale for picking Open Relay Project was "no account for this project, no account or config for a player." Surfaced it to the maintainer directly rather than picking a workaround unilaterally; the maintainer chose to sign up for a metered.ca account and fetch credentials from their REST API at runtime, accepting the account requirement.

Implementation: `TURN_CONFIG` is now an empty array populated at runtime rather than a hardcoded list. `fetchTurnCredentials(targetArray)` calls `https://<subdomain>.metered.ca/api/v1/turn/credentials?apiKey=<key>` (both currently placeholder empty strings pending the maintainer's actual dashboard values) and, on success, mutates `targetArray` in place — pushed with a 4s `AbortController` timeout, and logged to the existing diagnostics panel either way (refreshed count on success, falls-back-to-STUN-only reason on failure). `connectToRoom` calls it fire-and-forget (`fetchTurnCredentialsFn(TURN_CONFIG)`, no `await`) immediately before `joinRoomFn` — deliberately not blocking room-join on a network round trip. This relies on a fact confirmed directly by reading Trystero's source rather than assumed: `joinRoom()` (`torrent.mjs`) spreads the config object but not the arrays inside it, and `strategy.mjs`'s `ctx.config` keeps that exact same `turnConfig` array reference for the room's whole lifetime, read fresh by `peer.mjs` only at the moment a real peer connection is actually constructed (`initPeer(false, ctx.config)`, destructured at call time, not at `joinRoom()` time). Since establishing a peer connection requires the tracker-signaling handshake to complete first — observed taking seconds to over a minute in practice — a credential fetch that resolves after `joinRoomFn` is called still reaches the array well before it's ever read for a real connection, with no restructuring of `connectToRoom`'s synchronous calling convention needed.

`TURN_CONFIG` starting empty rather than keeping the stale entries matters on its own: Trystero's `hasTurnServer(config)` check treats "a `turnConfig` is present at all" as license to report the specific "check your TURN server credentials" wording — accurate only once real credentials are in there, actively misleading (implying a config problem when there's no TURN configured yet at all) while empty-but-non-configured.

Docs: none — same reasoning as every prior TURN round (§95), this is transport-layer wiring the cart-authoring API never sees.

`test/smoke.js`'s existing TURN regression check (§95, asserting a hardcoded array's contents) no longer applied to a runtime-fetched one; replaced with two checks matching the new mechanism: `connectToRoom` hands `joinRoomFn` the *exact* array `fetchTurnCredentialsFn` was given (by-reference, not a value snapshot at join time), and a fake fetcher's credentials — resolved asynchronously, after `joinRoomFn` was already called — are visible in that same array once awaited, proving the fire-and-forget path actually reaches its target instead of racing past it. `fetchTurnCredentialsFn` is injectable on `connectToRoom`'s options, the same override convention `joinRoomFn` and `voteTimeoutMs` already established, so this needed no real network call or real metered.ca credentials to verify. 3 clean full-suite runs.

Still open, tracked here rather than resolved: the maintainer's actual metered.ca subdomain and API key haven't been provided yet, so `METERED_SUBDOMAIN`/`METERED_API_KEY` remain empty placeholders and the app currently runs STUN-only in production (gracefully — `fetchTurnCredentials` logs "not configured" and returns, same as today's actual deployed behavior) until they're filled in.

## 104. Real metered.ca credentials wired in — and a stale-fetch diag bug §103's own placeholders had been hiding

The maintainer's metered.ca dashboard values arrived: `urlcade.metered.live` (their TURN REST API domain — issued under `metered.live`, not `metered.ca` itself, so §103's `METERED_SUBDOMAIN`/fixed-`.metered.ca`-suffix design was replaced with a single `METERED_TURN_HOST` holding the full host) and the API key. Both are now live constants in `multiplayer.js` rather than empty placeholders.

Turning the fetch from theoretical to real (previously every `test/smoke.js` run had `METERED_TURN_HOST`/`METERED_API_KEY` empty, so `fetchTurnCredentials` always took its "not configured" early return and never actually reached the network) surfaced two problems immediately:

1. **The sandbox's outbound proxy has no reason to allow an arbitrary new host.** Every non-mocked `hostMatch`/`joinMatch`/`connectToRoom` call in the smoke suite now fired a real `fetch()` at `urlcade.metered.live`, which the proxy correctly rejects (not on its allowlist) — surfacing as `net::ERR_TUNNEL_CONNECTION_FAILED` console errors that tripped the blanket "no console/page errors across the whole run" check, for reasons that had nothing to do with what any individual test was actually exercising. Fixed by routing that one endpoint through Playwright's `page.route()` once, for the whole test run, fulfilling it with a fake-but-valid credential response — this is shared test infrastructure (every test benefits, none of them should have to know about it), not something to thread through each test's own mock setup individually. This also means the real `fetchTurnCredentials` code path — not just the injectable test double — now runs for real in every relevant test, a strictly better guarantee than §103 shipped with.
2. **A real bug, found only once tests actually let a fetch resolve asynchronously across two different sessions**: `fetchTurnCredentials`'s `pushDiag()` calls write into `diagLog`, a page-global (not session-scoped) array — completely reasonable when `TURN_CONFIG` starts empty and never really gets populated (§103's actual shipped state), but once a real, slower fetch is in flight, a session that's already been closed can have its fetch resolve *after* a brand new session has opened, writing a stray "TURN credentials refreshed" line into a connection log that has nothing to do with it. Concretely: a test hosting a room with a `joinRoomFn` that throws synchronously still fires `fetchTurnCredentialsFn` before the throw is caught (the fetch isn't inside the same try/catch), and that session's own `session.leave` gets overwritten to a no-op in the error path — so there was no session-level signal available to suppress the eventual, now-orphaned resolution. Confirmed as a real, user-facing risk, not just a test artifact: the diagnostics panel is the same real in-UI connection log a player would read directly (this whole arc, §88 onward, started because a maintainer read one off a phone screen), so a real player closing one failed attempt and immediately starting a new one could see a confusing, out-of-context "TURN credentials refreshed" line bleed into the new attempt's log.

   Fixed with a staleness guard matching the idiom already used throughout this file (`cascadeId`, `lastConfirmedRemoteTick`): a module-level `diagStopGeneration` counter, bumped only by `stopDiag()` (every session teardown, `closeLobby`'s own call), not by `startDiag()`. `fetchTurnCredentials` captures the value in effect the instant it's fired (necessarily *before* the current session's own `startDiag()` has run, since `connectToRoom` fires the fetch and only its caller starts diag afterward) and re-checks it before each of its own `pushDiag` calls — skipping them if a teardown happened anywhere in between. This correctly keeps logging for a fetch that's still relevant to the session that's currently open (no teardown has happened yet), while silently dropping one whose owning session — or a later one — has already closed by the time it resolves. Bumping only on stop rather than on every start is what makes the ordering work: bumping on start too would have marked even a brand-new session's own still-in-flight fetch as stale the instant its `startDiag()` ran, moments after the fetch was fired.

Docs: none — same as §103, this is transport/diagnostics wiring the cart-authoring API never touches.

Verified: `test/smoke.js`'s existing diagnostics-panel checks (§185, unmodified) now pass again with the real fetch path live, plus the new `page.route()` stub covering every other test that touches a real session. 3 clean full-suite runs. The actual live metered.ca credentials themselves could *not* be end-to-end verified from this sandbox — its own outbound proxy denies `urlcade.metered.live` by policy (confirmed via the proxy's own status endpoint, not worked around), consistent with every other point in this arc where real WebRTC/TURN behavior has only ever been confirmable on a real device on a real network. That verification is now on the maintainer for the next real two-device attempt.

## 106. Lobby UX overhaul — first stage: the connection log collapses behind a native `<details>` instead of always showing once populated

The multiplayer party feature (§96-104) worked, but the UI hadn't caught up — confirmed directly with the maintainer after a successful real two-device TURN test, who asked for five concrete improvements. This is the first of five independently-shippable stages tackling them, sequenced the same way every prior multi-stage arc in this file has been (plan agreed with the maintainer via a written plan file before any code changed).

The connection log (§88) is genuinely valuable — it's literally what let this session root-cause the TURN relay bug in §103 by reading it off a phone screen with no devtools — but it's also very technical, and `renderDiag()` showed it (`#mpDiag`, `display:block`) the instant `diagLog.length` was nonzero, meaning every real party session showed it immediately. Wrapped the whole panel in a native `<details id="mpDiagWrap"><summary>Connection details</summary>...</details>`, collapsed by default (no `open` attribute) — zero new JS state needed, since the browser's own disclosure widget already does exactly "hidden until the user asks for it, one tap to reveal." `renderDiag()` needed exactly one change: retarget its `style.display` toggle from `#mpDiag` to the new `#mpDiagWrap` (still gating "is there anything to show at all," never the open/closed state itself, which `<details>` and its own `.open` property now own).

One behavior added beyond the literal ask: `handleSessionStateChange` (and `startSession`'s own synchronous-error path, which never reaches that listener) now sets `mpDiagWrap.open = true` when `session.state === 'error'` — a player who actually hits a real connection problem shouldn't have to know "Connection details" exists first. `stopDiag()` also resets `.open = false` on every session teardown, so an error-triggered expand from one party doesn't leak into the next one's panel.

Docs: none — UI-only, no cart-authoring API surface change (CLAUDE.md's upsert rule).

Verified via `test/smoke.js`: existing `#mpDiag` visibility checks retargeted to `#mpDiagWrap` (mechanical); new checks confirm the panel stays collapsed even once populated, that tapping the summary expands it, and that a real connection error auto-expands it. Full suite green.

## 107. Lobby UX overhaul — second stage: `#mpOverlay` goes full-screen

`#mpOverlay` was a small centered card with a dark scrim (`.mp-overlay{align-items:center;justify-content:center}` + `.mp-card{max-width:360px}`) — functional on desktop, cramped on mobile, and not what "integrated with the shelf" (the next stage) would have room for. Modeled the replacement directly on `#inspectWrap`/`.inspect-topbar`/`.inspect-body`'s own existing full-screen recipe (safe-area-aware padding, flex-column, scrollable body under a fixed topbar) rather than inventing a second pattern — `#mpOverlay .mp-card` now fills the viewport (`width/height:100%`, `height:100dvh`, no border-radius/shadow/max-width), with a `.mp-topbar` (just the Close button — no duplicate title, since every state's own `<h3>` already provides one) and a `.mp-body-scroll` wrapping `#mpBody` and the diagnostics `<details>`.

The one real constraint: `#identityOverlay` (the name/avatar picker) reuses the exact same `.mp-overlay`/`.mp-card` base classes and has no reason to go full-screen — a tiny form doesn't need it. Every new rule is scoped strictly by `#mpOverlay`'s own ID, never touching the shared base rules, so `#identityOverlay` is untouched. No JS changes were needed for open/close mechanics — every `classList.add/remove('active')` call already worked unmodified, since `.mp-overlay.active{display:flex}` still governs visibility identically for both overlays regardless of size.

One now-dead-but-harmless corner: `initMultiplayerUI`'s scrim-click-to-dismiss listener (`e.target.id === 'mpOverlay'`) has no visible scrim left to click now that the card fills the whole overlay — left in place (commented) rather than removed, since it's free and still correct if a future revision ever leaves margin around the card again.

Docs: none.

Verified: new `test/smoke.js` check asserting `#mpOverlay .mp-card` fills the viewport at both a desktop and a narrow mobile size (`page.setViewportSize`), while `#identityOverlay .mp-card` stays a small centered card at the same two viewports — the concrete regression test for the ID-scoping requirement, not just something to eyeball. Every existing click-handler test kept passing unmodified (only ancestor structure changed, no IDs/classes on the interactive elements themselves). Playwright screenshot confirms the redesigned shell visually.

## 108. Lobby UX overhaul — third stage: the queue and "Add a game" list become real shelf cards

The party queue (§97) was a plain list of text rows — a name, an "added by" caption, two small buttons. The ask was for it to feel like part of the game shelf, not a dropdown apart from it. `runtime.js`'s `buildCardThumbnail(cart)` already does exactly the rendering needed (decodes a cart, renders its post-on_init first frame to an offscreen canvas) — it just wasn't exported. Added it to the `export {}` list and imported it into `multiplayer.js`.

The one real gap: a queue item only ever carries `{fragment, name, author, maxPlayers, addedBy}` over the wire (`party-queue`'s own shape) — never a decoded cart — so a thumbnail has to be built locally. New fragment-keyed `thumbCache`/`ensureThumbnail(fragment)`, mirroring the existing `mpCapableCartsList`/`ensureMultiplayerCapableCartsLoaded()` lazy-decode-then-`renderLobby()` idiom exactly. `renderQueueList`/`renderAddGameSection` now emit real `.cart`/`.cart-thumb-link`/`.cart-thumb`/`.cart-body` markup (the shelf's own classes, reused rather than duplicated) in a dense auto-fill grid matching `#cartList`'s own layout — a maintainer choice, confirmed directly (`AskUserQuestion`) over the alternative of fewer, wider single-column cards. Existing `.mp-queue-item`/`.mp-queue-play`/`.mp-queue-remove` class names and `data-queue-id` attributes were kept unchanged so `renderLobby()`'s event-delegation code needed no edits; "Add a game" cards became a single clickable `<button class="cart mp-cart-add">` per item, reusing `.playbtn`'s visual rule (broadened from `a.playbtn` to `.playbtn` — dropping the tag qualifier — since a card click here has to call `session.startMatch()`/`addToQueue()` through JS, never trigger a real hash navigation the way the shelf's own `<a href>` does, which would desync the party).

A single cached `<canvas>` can only ever have one DOM parent — real risk here, since nothing dedupes the queue and "Add a game" lists against each other, so the same fragment can legitimately need a thumbnail in two places in one render. `cloneThumbCanvas()` copies the cached canvas's pixels into a fresh canvas per slot (`drawImage`, not `cloneNode` — cloning a canvas element copies neither its bitmap nor its context) rather than moving the one cached node around.

Docs: none.

Verified: existing queue/add-game button tests passed unmodified (class names preserved). New checks: a queued item (seeded from a real loaded cart) shows a real `<canvas>` thumbnail once `ensureThumbnail` resolves; a cart that fails to decode degrades gracefully, same as `renderMenu()`'s own shelf fallback. Screenshot confirms real thumbnails rendering in the dense grid, matching the shelf's own look.

## 109. Lobby UX overhaul — fourth stage: a 3-second "Get Ready" countdown before a match actually starts

`beginSyncedMatch` used to hide the overlay and call `startGame()` immediately — zero lead-in. Added a countdown phase in front of it: `connectToRoom` gained a `countdownMs = 3000` option (same injectable-for-tests shape `voteTimeoutMs` already established, stashed on `session` since `beginSyncedMatch` is module-level and only ever receives `session`, not the original opts closure). Per the maintainer's own confirmed choices (`AskUserQuestion`): 3 seconds, showing who and what (peer avatar/name, the target game's real name + thumbnail via Stage 3's `ensureThumbnail` cache, plus a large counting-down number) rather than a bare number, and closing the panel mid-countdown just hides it — the match still starts on schedule, matching the existing "X only hides, Leave Party actually leaves" posture everywhere else in the lobby.

No wire message was needed for this, by design: both peers already reach `beginSyncedMatch` independently off the same `onMatchStart` event (one immediately from the sender's own `session.startMatch()`/cascade resolution, the other after the `party-start` wire round-trip) — the same "deterministic and symmetric" reasoning `assignSlot()` already relies on for slot assignment. A fixed local countdown on each side lands both peers' actual `startGame()` calls within a fraction of a second of each other, comfortably inside what the existing 8-tick rollback ring buffer (~133ms at 60fps) already tolerates day to day.

`runCountdown(session, fragment, generation)` ticks off real elapsed time (a 100ms poll against `Date.now() - startTime`, not a fixed once-per-second `setTimeout` chain) so the displayed number is correct regardless of how `countdownMs` divides into whole seconds — `test/smoke.js` injects durations as short as 50ms. It checks the existing `matchGeneration` counter on every tick (the same one `beginSyncedMatch`'s post-`startGame()` guard already used) rather than needing any new invalidation call site — the two existing `stopSync()` callers (peer-left, a fresh cascade starting) already bump it, so a peer leaving mid-countdown aborts the countdown exactly like a stale in-flight `startGame()` already got discarded.

The peer-avatar-canvas and thumbnail-canvas "emit a placeholder, fill it in after innerHTML exists" logic was previously inlined in `renderLobby()` — factored out into a shared `attachDynamicSlots(session, body)`, since the countdown view needs both too and bypasses `renderSessionBody()`'s normal dispatch entirely (there's no session-state branch for "counting down").

Docs: none.

Verified: every existing test that clicks Play and expects a match to start soon after now injects a short `countdownMs` (50ms), the same convention `voteTimeoutMs`'s own tests established, avoiding real multi-second waits. New tests: the countdown view actually counts down through more than one distinct number and shows the peer's name and the game's real thumbnail (not just a bare number); a peer leaving mid-countdown (deliberately long injected duration) never lets a stale countdown go on to start a match with no one there — the direct analog of the existing `matchGeneration` staleness test covering the `startGame()` await gap; closing the panel mid-countdown hides it immediately but the match still starts on schedule. Screenshot confirms the countdown view's layout.

## 110. Lobby UX overhaul — fifth and final stage: transition animations scoped to actual view-kind changes, not every render

The lobby's render model is a full `innerHTML` replace on every `renderLobby()`/`runCountdown()` call, including routine updates that aren't a state change at all — a queue item added, the peer's avatar arriving, a thumbnail resolving, the countdown number ticking down. Animating all of those would read as flicker, not the "more friendly" transition actually asked for. New `renderKindOf()` distinguishes the *visual* branch being shown — `lobbyMode` before a session exists, `'cascade'` when connected with a vote in progress, `'countdown'` during Stage 4's countdown (tracked via a new `inCountdown` flag, since that state doesn't live in `session` at all), otherwise raw `session.state` — from `session.state` alone, which can't make those distinctions by itself.

`maybeAnimateBody(body)`, called right after every `#mpBody` innerHTML replace (all three `renderLobby()` branches, `runCountdown()`'s own per-tick render), compares the new kind against a module-level `lastRenderedKind` and only on an actual change replays the entrance animation via the exact remove/reflow/re-add restart idiom `runtime.js`'s `startGame()`/`showMenu()` already use (`classList.remove`, `void body.offsetWidth` to force a reflow, `classList.add`) — reusing the existing `viewIn` keyframe (index.html, already used by `.view.active`) rather than inventing a second animation system, bound to a new `.mp-body-anim` class (not `.view.active` itself, to avoid confusion with `#mpOverlay.active`'s unrelated meaning). `.mp-overlay.active` also picked up the same `viewIn` keyframe directly — a separate, additive line item covering the modal's own open/close, safe for both `#mpOverlay` and `#identityOverlay` since it's just a fade+slight-rise independent of size. Both stay wrapped in the existing `@media (prefers-reduced-motion: no-preference)` guard already used elsewhere in this file.

Docs: none.

Verified: `test/smoke.js` exposes a new `getLobbyAnimationTriggerCount()` debug getter — asserting "a class was present" after one render can't distinguish a re-trigger from having never changed, so a counter that only increments on an actual kind change is the direct, observable signal needed. New checks walk a real sequence (choice → waiting-for-peer → connected → cascade) confirming the counter increments exactly once per real transition, and confirm a queue mutation within the same `'connected'` kind does *not* retrigger it. Full suite green, no reduced-motion-specific test handling needed (the pre-existing `.view.active{animation:viewIn}` rule already ran for real throughout the whole suite with zero flakiness, since every test asserts DOM/class state, never mid-animation pixel positions).

With this stage shipped, all five of the maintainer's requested lobby UX improvements are in: the connection log collapses by default, the queue and "Add a game" list use real shelf cards, `#mpOverlay` is full-screen, a countdown leads into every match, and state transitions animate instead of jumping. `test/smoke.js` passed clean throughout (292 checks by the end of this arc); per CLAUDE.md's upsert rule, none of it touched the cart-authoring API surface, so no skill/spec/learn-site doc updates were needed anywhere in this arc.
